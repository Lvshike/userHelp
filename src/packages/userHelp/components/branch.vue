<template>
  <div class="branch">
        ...
        <div class="box">
          <div v-if="listOrgData.length" class="orgData">
            <div class="listItem" v-for="item in listOrgData" :key="item.id">
              <van-checkbox  :class="{'left':true,'checked':item.choice}" @click="select(item)" :value="!!item.choice">
                <p :class="{'disabled':item.choice}" >{{item.name}}</p>
              </van-checkbox>
              <p v-if="item.fblowerLevel" :class="{'right':true,'disabled':item.choice}" >
                <i class="line"></i>
    ...
    <div v-if="selectShow" class="selectList" >
      <flow-title :back="true" :title="'已选择'+num()+'个分部'" @back="back" />
      <div class="selectBox">
    ...
    <div v-if="isSearch" class="searchDetail" >
      <flow-title :back="true" title="搜索分部" @back="searchBack" />
...
<script>
import { 
  defaultOrgHandleBranch as defaultOrg,
  currentOrg,
  headOrg,
  OrgQuerysFB
} from "../../../api/api.js";
...
export default {
  ...
  mounted(){
    ...
    this.isSideslip()
    document.querySelector(".branch .search input").focus()
  },
  beforeDestroy(){
  ...
  methods:{
    ...
    remoteMethod(value){
      if(value){
        OrgQuerysFB({ name: value },this.appId).then(res=>{
          if (!res.success) return
          let data = res.data.filter(t=>t&&t.type===2)
          this.selectData.forEach(t=>{
            for(let i=0;i<data.length;i++){
              if (data[i].id === t.id) data[i].choice = true
              continue
